...
SmoothPlasmaTac <- function(time, activity){
  ...
  # From the just beyond peak to 4.5 minutes, discarding
  # right extreme after 3 min:
  t.max <- tau[i.max]
  idx <- tau > t.max & tau <= 4.5
  temp <- locpoly(tau[idx],ca[idx],degree=3,bandwidth=.4,gridsize=50)
  idx <- tau > tau[i.max+5] & tau <= 3
  temp$x <- temp$x[idx]
  temp$y <- temp$y[idx]
  t.last <- tail(temp$x,1)
  ans <- rbind(ans,cbind(temp$x,temp$y))
  ...
  idx <- tau > t.last & tau <= 20
  temp$x <- temp$x[idx]
  temp$y <- temp$y[idx]
  t.last <- tail(temp$x,1)
  ans <- rbind(ans,cbind(temp$x,temp$y))
  # From 20 to the end, using a variable bandwidth, using data from 2
